...
def Potential(x, R, R_charges):
    ...
    d = 1 / torch.norm(
        (
            x[:, None, :]
            - (R * R_charges[:, None]).repeat(1, dimension // R.shape[-1])[None, :, :]
        ).view(batchsize, -1, R.shape[-1]),
        dim=-1,
    )
    D = (
        1
        / torch.norm(R[:, None, :] - R[None, :, :], dim=-1)
        * (R_charges[None, :] * R_charges[:, None])
    )
...
def Gradient(x, R, net):
    ...
    grad = torch.autograd.grad(
        Psi, x, create_graph=True, grad_outputs=torch.ones(batchsize)
    )[0]
...
def Gridgenerator(dimensions, sites, interval):
    ...
    G = torch.meshgrid(
        [torch.linspace(interval[0], interval[1], sites) for i in range(dim)]
    )
    X = []
    for i in range(dim):
    ...
    for _ in range(zerodim):
        X = (
            torch.cat((X.transpose(0, 1).flatten(), torch.zeros(sites ** (dim))))
            .view(-1, sites ** (dim))
            .transpose(0, 1)
        )

    index = np.append(
        np.where(np.array(dimensions))[0],
        np.where(np.logical_not(np.array(dimensions)))[0],
    )
...
def fit(batch_size=2056, n_el=1, steps=2500, epochs=4, RR=None, RR_charges=None):
	...
	for _ in range(epochs):
		...
		for step in range(steps):
            ...
            # V     = Potential(X,RR,RR_charges)
            # E     = (
            #   torch.mean(torch.sum(grad_X**2,dim=1)/2+Psi**2*V)/torch.mean(Psi**2)
            # ).item()#*27.211386
            # print("E_grid = "+str(E))
            ...
            # ax.scatter(
            #   X[:,0].detach().numpy(),X[:,1].detach().numpy(),
            #   Psi.detach().numpy()**2,marker='.'
            # )
            # plt.show()
