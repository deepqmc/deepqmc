...
def HMC_ad(dist,stepsize,dysteps,n_walker,steps,dim,push,startfactor=1,T=1,presteps=200):
	...
	for i in range(steps+presteps):
		...
		while (len(na)/n_walker) > (1-ac) and count<10:
			...
			ratio[na] = (disttrial/distwalker[na])*(torch.exp(-0.5*(torch.sum(v_trial[na]**2,dim=-1)-torch.sum(v_walker[na]**2,dim=-1))))
