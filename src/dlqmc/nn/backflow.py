...
from .base import SSP, DistanceBasis, conv_indexing, get_log_dnn, pairwise_distance


class Backflow(nn.Module):
    def __init__(self, n_up, n_down, n_interactions, basis_dim, n_layers):
        super().__init__()
        self.n_up = n_up
        self.n_down = n_down
        ...
        for xs in [rs[:, : self.n_up].clone(), rs[:, self.n_up :].clone()]:
            for i, interaction in enumerate(self.interactions):
                ...
                *batch_dims, n_elec, n_elec, basis_dim = dists_basis.shape
                ...
                ys = (
                    Ws.view(*c_shape) * (xs[:, c_j].view(*c_shape) - xs[:, :, None, :])
                ).sum(dim=2)
                xs = debug[i] = xs + ys
            t.append(xs)
