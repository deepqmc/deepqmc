...
for epoch in range(5):
	...
	r     = torch.norm(X,dim=1)
	Psi   = net(X)
	E     = torch.mean(torch.sum(gPsi**2,dim=1)/2+Psi**2*(-1/r))/torch.mean(Psi**2) #energy still is somewhat strange, cause note very stable even if functions look similar
