...
#	v_te2 = vel + stepsize*get_grad_Psisquare(net,pos.detach())*(-1)/2
	v_te2 = vel - stepsize*torch.autograd.grad(dist(pos),pos,create_graph=True,retain_graph=True,grad_outputs=torch.ones(len(pos)))[0]
...
#	v_te  = v_te2 - stepsize/2*get_grad_Psisquare(net,p_te.detach())*(-1)
	v_te  = v_te2 - stepsize*torch.autograd.grad(dist(pos),pos,create_graph=True,retain_graph=True,grad_outputs=torch.ones(len(pos)))[0]
	return p_te,vel.detach().numpy()


def HMC(dist,stepsize,dysteps,n_walker,steps,dim,startfactor=0.2,T=0.3,presteps=0):
...
start=time.time()
POS=np.array(HMC(lambda x: -torch.log(net(x)**2),0.2,5,n_walker,n_steps,2,T=0.01).detach())
end=time.time()
print("HMC done")
...
start=time.time()
POS = np.array(metropolis(lambda x: -torch.log(net(x)**2),np.random.normal(size=2),0.3,n_steps,dim=2,n_walker=n_walker,interval=None,T=0.005).detach())
end=time.time()
...
start=time.time()
POS=np.array([pysgmcmcsampler(dist=lambda x: -torch.log(net(x)**2),lr=0.001,num_samples=1000).reshape(-1,2) for i in range(3)]).reshape(-1,2)
end=time.time()
print("pysgmcmc done")
print(np.round(end-start,1))
#_________________________________________________________________
