...
def HMC(dist,stepsize,dysteps,n_walker,steps,dim,push,startfactor=0.2,T=0.1,presteps=0):
	...
	for i in range(steps+presteps):
		...
		trial,v_trial = dynamics(dist,walker,stepsize,dysteps,push)
...
#pos = HMC(f,1,1,10,1000,1,push=0.3,T=1).detach().numpy()
#print(pos)

def update_hist(num, pos, line):	
	hist=np.histogram(pos[:num],range=[-5,5],density=True,bins=100)
...
line_ani = animation.FuncAnimation(fig1, update_hist, 50000, fargs=(pos, q),interval=3, blit=True)
