...
def fit(batch_size=2056,steps=15,epochs=4,R1=1.5,R2=-1.5,losses=["variance","energy","symmetry"]):
	...
	for epoch in range(epochs):
		...
		for i in range(1):
			G=torch.meshgrid([torch.linspace(-5,5,40*(i+1)),torch.linspace(-5,5,40*(i+1)),torch.linspace(-5,5,40*(i+1))])
			x=G[0].flatten().view(-1,1)
			y=G[1].flatten().view(-1,1)
			z=G[2].flatten().view(-1,1)
...
for R in Rs:
	psi,e = fit(batch_size=128,steps=10000,epochs=2,losses=["energy","symmetry"],R1=R/2,R2=-R/2)
	E_min.append(e)
	Psi_min.append(psi)
